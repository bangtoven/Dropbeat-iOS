...
- (UITableView *)buildTableView{
    ...
    if(n > 0){
        ...
        if (height > newRect.size.height - heightOffset) {
...
- (IBAction)confirmButtonPressed:(UIButton *)sender {
    [self dismiss:^{
        if(self.selectedIndexPath && [self.delegate respondsToSelector:@selector(didConfirmWithItemAtRowInDYAlertPickView:)]){
            [self.delegate didConfirmWithItemAtRowInDYAlertPickView:self.selectedIndexPath.row];
        }
    }];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.dataSource respondsToSelector:@selector(titleForRowInDYAlertPickView:)]) {
        cell.textLabel.attributedText = [self.dataSource titleForRowInDYAlertPickView:indexPath.row];
    }
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(self.tapPickerViewItemToConfirm && [self.delegate respondsToSelector:@selector(didConfirmWithItemAtRowInDYAlertPickView:)]){
        [self dismiss:^{
            [self.delegate didConfirmWithItemAtRowInDYAlertPickView:indexPath.row];
        }];
    }
}
@end
