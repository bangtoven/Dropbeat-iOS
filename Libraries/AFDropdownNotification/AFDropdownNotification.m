...
-(id)init {
    ...
    if (self) {
        ...
        _titleLabel.textColor = self.isDarkTheme ? [UIColor whiteColor] : [UIColor blackColor];
        ...
        _subtitleLabel.textColor = self.isDarkTheme ? [UIColor whiteColor] : [UIColor blackColor];
...
-(void)presentInView:(UIView *)view withGravityAnimation:(BOOL)animation {
    ...
    if (!_isBeingShown) {
        ...
        NSInteger titleHeight = MIN(44, [_titleLabel.text boundingRectWithSize:CGSizeMake(textWidth, 999) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue-Medium" size:kDropdownTitleFontSize]} context:nil].size.height);
        NSInteger subtitleHeight = 0;//[_subtitleLabel.text boundingRectWithSize:CGSizeMake(textWidth, 999) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:kDropdownSubtitleFontSize]} context:nil].size.height;
        NSInteger notificationHeight = 64;//(statusBarHeight + kDropdownPadding + titleHeight + (kDropdownPadding / 2) + subtitleHeight + kDropdownPadding);
...
//        }
        ...
        _notificationView.backgroundColor = self.backgroundColor;//[UIColor clearColor];
        ...
        if (NSFoundationVersionNumber > NSFoundationVersionNumber_iOS_7_1) {
            UIVisualEffect *visualEffect = [UIBlurEffect effectWithStyle:self.isDarkTheme ? UIBlurEffectStyleDark : UIBlurEffectStyleLight];
        ...
        _imageView.frame = CGRectMake(kDropdownPadding, (notificationHeight / 2) - (kDropdownImageSize / 2) + (statusBarHeight / 2), kDropdownImageSize, kDropdownImageSize);
        
        if (_image) {
            if (_image.size.width < kDropdownImageSize && _image.size.height < kDropdownImageSize) {
        ...
        _titleLabel.frame = CGRectMake(kDropdownPadding + kDropdownImageSize + kDropdownPadding, (notificationHeight / 2) - (titleHeight / 2) + (statusBarHeight / 2), textWidth, titleHeight);
        ...
        _topButton.frame = CGRectMake(_titleLabel.frame.origin.x + _titleLabel.frame.size.width + kDropdownPadding, statusBarHeight + (kDropdownPadding / 2), kDropdownButtonWidth, kDropdownButtonHeight);
